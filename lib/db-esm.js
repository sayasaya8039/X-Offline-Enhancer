...
async function importAll(dump, { onProgress } = {}) {
  if (!dump || typeof dump !== 'object') throw new Error('invalid dump');
  ...
  const images = Array.isArray(dump.images) ? dump.images : [];
  const videos = Array.isArray(dump.videos) ? dump.videos : [];
