...
function openDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // v3: dedicated image blob store — no migration of existing base64 imageCache; cache limit purge will drop it.
      if (!db.objectStoreNames.contains(IMAGE_STORE_NAME)) {
        const iStore = db.createObjectStore(IMAGE_STORE_NAME, { keyPath: 'key' });
        iStore.createIndex('threadId', 'threadId', { unique: false });
      }
    };
...
async function addThread(thread) {
  // imageCache inline storage removed in v3 — use image_blobs store instead
  if (thread && thread.imageCache) delete thread.imageCache;
 ...
 * imageCache is stripped per thread — use image_blobs store instead.
  ...
  return withRetry(async () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const t of threads) {
        if (!t) continue;
        if (t.imageCache) delete t.imageCache;
        store.put(t);
        count++;
      }
    });
...
async function getAllThreads() {
  console.warn('[XOfflineDB] getAllThreads() loads every thread into memory — prefer getAllThreadsMeta() for listing views.');
  return withRetry(async () => {
    const db = await openDB();
    return new Promise((resolve, reject) => {
      const tx = db.transaction(STORE_NAME, 'readonly');
 ...
 * @param {string} threadId
 * @param {number} index - image index within the thread
 ...
 * @param {string} threadId
 * @param {Array<{index: number, blob: Blob, mimeType: string}>} items
 */
 ...
 * @param {string} threadId
 * @returns {Promise<Array<{index: number, blob: Blob, mimeType: string}>>}
 */
  ...
  return withRetry(async () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        results.push({ index: v.index, blob: v.blob, mimeType: v.mimeType });
        cursor.continue();
      };
      request.onerror = () => reject(request.error);
    });
