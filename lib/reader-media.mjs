...
export function buildVideoBlobMaps(tweets, videoRecords, createObjectURL = (blob) => URL.createObjectURL(blob)) {
  ...
  for (const record of videoRecords || []) {
    ...
    normalizedRecords.push({ record, objectUrl });
  }

  const hasVideoTweetIndexes = (tweets || [])
    .map((tweet, index) => (tweet?.hasVideo ? index : -1))
    .filter((index) => index >= 0);

  // Safe fallback only when there is exactly one saved video and one target tweet.
  if (normalizedRecords.length === 1 && hasVideoTweetIndexes.length === 1) {
    fallbackByTweetIndex.set(hasVideoTweetIndexes[0], normalizedRecords[0].objectUrl);
  }
